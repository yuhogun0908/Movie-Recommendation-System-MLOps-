...
class Process_RawData:
    ...
    def copy_latest_directory(self):
        dirpath = osp.join(get_project_root(), "DB/RAW_DATA", "latest")
        if not os.path.isdir(dirpath):
            os.makedirs(dirpath, exist_ok=True)
        print("  >> Copy the RAWDATA into latest directory... Wait....")
        copy_tree(src=self.output_path, dst=dirpath)
        print("  >> Complete saving kafka data to {}".format(dirpath))

        return

    def del_prev_save_dir(self, dir_name):
        #cur_latest_dir = self.get_latest_path()
        #print("  @@@@@ 181818 cur_latest_dir: {}".format(cur_latest_dir))
        filepath = osp.join(osp.join(get_project_root(), "DB/RAW_DATA"), dir_name)
        if not os.path.isdir(filepath):
            print("  >> No {} directory exist...".format(filepath))
            return
